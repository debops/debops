...
``name``
  Required. Defines the name of the section which a given set of options belong
  to.

``state``
  Optional. If not specified or ``present``, a given configuration section
  will be present in the generated configuration file. If ``absent``, a given
  section will be removed from the configuration file. If ``comment``, the
  section will be present but commented out.

  If the state is ``init``, the section will be "primed" in the configuration
  pipeline, but it will be commented out in the generated configuration file.
  Any subsequent configuration section with the same ``name`` will switch the
  state to ``present`` - this is used to define the default sections in the
  role which can be changed via the Ansible inventory.

  If the state is ``ignore``, a given configuration section will not be
  evaluated during role execution. This can be used to activate configuration
  sections conditionally.

``comment``
  Optional. String or YAML text block with comments to be included before the
...
``raw``
  Optional. A YAML text block with the raw contents of the section. Takes
  precedence over ``options``.

The configuration options in turn accept the following parameters:
...
``state``
  Optional. Same as for a section above.
...
``raw``
  Optional. Same as for a section above. Takes precedence over ``value``.

``quote``
  Optional. Whether the ``value`` should be quoted in the generated
  configuration.
...
manual pages. Note that the files are processed in alphanumeric order.
...
``sections``
  Optional. List of YAML dictionaries defining sections and their options using
  the same syntax as used in :ref:`networkd__ref_configuration` above. Note
  that if the ``raw`` parameter is defined, this parameter will be ignored.
...
``comment``
  Optional. String or YAML text block with comments to be included in the
  generated configuration file.

``owner``
  Optional. The user who should own the generated unit file. Defaults to
  ``root``.

``group``
  Optional. The group which should own the generated unit file. Defaults
  to ``systemd-network``.

``mode``
  Optional. The permissions of the generated unit file. Defaults to
  ``0644`` (i.e. the ``owner`` can read and write, everyone else can
  read the unit file). If you create a unit file which contains
  sensitive data (e.g. a WireGuard device, which will typically
  contain private keys), the permissions should probably be set to
  something more restrictive like ``0640``.
