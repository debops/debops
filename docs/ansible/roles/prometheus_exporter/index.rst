...
``debops.prometheus_exporter`` installs ``prometheus-*-exporter`` Debian packages
and releases exporters from repositories, and it uses Ansible delegation to
configure access from the Prometheus server using file-based service discovery
mechanism. You can use ``debops.prometheus_server`` role to manage the Prometheus
server itself.

``debops.prometheus_exporter`` installs and configure by default node exporter.
