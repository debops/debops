...
``debops.influxdb`` role is the "client" part - it uses Ansible delegation
to configure users and databases in local or remote InfluxDB servers. You
can use ``debops.influxdb_server`` role to manage the InfluxDB server itself.
