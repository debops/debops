...
- password lookups, either from current role, or using known location of
  ...
  debops.mariadb_ role can manage an user account in the database with
...
- secure file storage, for example for application keys generated on remote
  hosts (debops.boxbackup_ role retrieves client keys for backup
...
- secure workspace (debops.boxbackup_ role, again, uses the secret directory
...
- simple centralized backup (specific roles like debops.sshd_,
  debops.pki_ and debops.monkeysphere_ have separate task lists that
