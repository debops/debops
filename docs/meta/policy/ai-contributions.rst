...
Whether content wholly generated by an LLM is copyrightable remains
unsettled across jurisdictions. Where it is not copyrightable, it carries
no license terms of its own and cannot be reliably licensed into the
project under its existing terms. Copyright infringement does not require
intent: failing to recognize that generated output reproduces third-party
material does not remove the risk to the project.
...
``Generated-By: LLM (Claude)`` — for additional transparency. Contributors
MAY note in the commit body how the tool assisted — for example proposed an
implementation, suggested changes to existing code, or drafted new tests —
when that context helps maintainers during review. Automated
