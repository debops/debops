...
a reliable, secure and reproductible management of servers and workstations "in
...
switch from Debian Stable to Debian Oldstable release during their lifecycle.
...
release and interested userbase.

For other systems (workstations, laptops, etc.) preferred OS distributions are
Debian derivatives like Ubuntu Linux, or others, depending on the choice of the
administrator. Support for these distributions may vary, any required changes
to make the roles and playbooks work on other Debian-based systems are welcome.
...
here are guaranteed to be stable (without any changes apart from the security
updates), signed and trusted by Debian Developers. Debian Software Repository
...
Debian Software Repository, upstream repositories should require a condition
...
in Ansible, and not using external scripts provided by the vendor. This ensures
...
considered more reliable than software installed from source repositories.
However different package managers might have different security practices,
...
Software Repository should have priority if possible.
